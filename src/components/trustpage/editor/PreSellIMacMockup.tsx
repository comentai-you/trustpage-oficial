...
const PreSellIMacMockup = ({ content, ownerPlan }: PreSellIMacMockupProps) => {
  ...
  // Base dimensions for the iMac mockup (rendered at fixed size, then scaled)
  const BASE_WIDTH = 520;
  const BASE_HEIGHT = 340;
  const STAND_HEIGHT = 80;
  const TOTAL_HEIGHT = BASE_HEIGHT + STAND_HEIGHT;

  const viewportWidth = 1280;
  const screenWidth = 512; // inner screen content area
  ...
  useEffect(() => {
    const updateScale = () => {
      ...
      const availableHeight = parentEl.offsetHeight;

      const scaleW = availableWidth / BASE_WIDTH;
      const scaleH = availableHeight / TOTAL_HEIGHT;
    ...
    <div ref={containerRef} className="w-full h-full flex items-center justify-center overflow-hidden">
      ...
      >
        ...
        <div
          ...
          style={{
            boxShadow:
              "0 25px 60px -15px rgba(0, 0, 0, 0.4), 0 0 0 1px rgba(0,0,0,0.1)",
          }}
        >
          {/* Screen bezel */}
          <div className="w-full h-full bg-black rounded-lg p-[2px]">
            ...
            >
                ...
                <div className="flex-1 mx-4">
                  <div className="bg-zinc-700 rounded-md h-4 flex items-center justify-center">
                    <span className="text-[8px] text-zinc-400">
                      {PUBLIC_PAGES_DOMAIN}/p/sua-presell
                    </span>
                  </div>
                </div>
              </div>
              ...
              >
                <style>
                  {`.imac-presell-scroll::-webkit-scrollbar { display: none; }`}
                </style>
